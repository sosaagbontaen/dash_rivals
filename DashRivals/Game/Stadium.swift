...
final class Stadium {
    ...
    private func startingBlock(atLaneX x: Float) -> SCNNode {
        ...
        let rail = SCNBox(width: 0.07, height: 0.05, length: 0.62, chamferRadius: 0.01)
        rail.materials = [dark]
        let railNode = SCNNode(geometry: rail)
        railNode.position = SCNVector3(0, 0.03, 0)
        block.addChildNode(railNode)

        for (i, dz) in [Float(-0.10), Float(-0.34)].enumerated() {
